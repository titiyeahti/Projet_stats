...
l = function(sigma, lambda, x, imax) {
  somme = 0
  for (i in seq(0, imax)) {
    somme = somme + exp(-1 * i) * lambda ** i / factorial(i) * (1 / (sqrt(2 * pi) * sigma)) ** length(x)
    sousSomme = 0
    for (k in seq(1, length(x))) {
      sousSomme = sousSomme + (x[k] - 100 / (1 + i)) ^ 2 / (2 * sigma ^ 2)
    }
...
f = function(x) { return(l(x, lambda, vec, imax)) }
...
En appliquant la formule précédemment donnée, on trouve
